...
def preprocess_image(image_path):
    try:
        ...
        img = cv2.resize(img, (800, 800))  # Reduce memory usage
        ...
        if file:
            ...
            if image is not None:
                ...
                result = (f"✅ Marvel Rivals purchase detected!\n\nDetails:\n" +
                          "\n".join(f"{k}: {v}" for k, v in details.items())) if found else \
                         "❌ No Marvel Rivals purchase found in the uploaded image."
            else:
                result = "⚠️ Error processing the uploaded image."
            os.remove(file_path)
